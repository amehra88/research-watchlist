...
slug to a directional sign (-1 bearish/challenge, 0 neutral, +1 bullish/confirm).
...
`load()` fails loud (ValueError) if any watchlist theme slug is missing from
theme_polarity.yaml, or if any value there isn't in {-1, 0, 1} -- a new theme
added to the watchlist vocabulary must never silently default to a polarity.
