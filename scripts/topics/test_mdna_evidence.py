...
No pytest in this env — run directly:
    python3 scripts/topics/test_mdna_evidence.py
"""
import os
import sys
import tempfile
...
def test_claims_for_ticker_never_diffs_across_form_types():
    ...
    q1 = {"ticker": "X", "filed_date": "2026-02-01", "form_type": "10-Q", "document_id": "q1",
          "mdna": "Quarterly paragraph one that is long enough to pass the minimum word count filter for a block of prose here and more words to be safe about the twenty five word floor."}
    k = {"ticker": "X", "filed_date": "2026-03-01", "form_type": "10-K", "document_id": "k",
         "mdna": "Annual paragraph that is entirely different and also long enough to pass the minimum word count filter for a block of text with extra words appended for the floor."}
    q2 = {"ticker": "X", "filed_date": "2026-05-01", "form_type": "10-Q", "document_id": "q2",
          "mdna": q1["mdna"] + "\nA second quarterly paragraph that is new this quarter and long enough to pass the minimum word count filter for prose with a few extra words appended."}
    ...
    assert me.form_family("10-K/A") == "10-K" and me.form_family("10-Q/A") == "10-Q" and me.form_family("20-F") == "20-F"
