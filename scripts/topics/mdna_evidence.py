...
question on the same topic.
  ...
  python3 scripts/topics/mdna_evidence.py --dry-run
  python3 scripts/topics/mdna_evidence.py --ticker ADI
  python3 scripts/topics/mdna_evidence.py
"""
...
def form_family(form_type: str) -> str:
    """10-K and 10-K/A are one series; 10-Q and 10-Q/A another. Anything else is its own."""
    f = (form_type or "").upper().strip()
    for fam in ("10-K", "10-Q"):
        if f == fam or f.startswith(fam + "/") or f.startswith(fam + "T"):
            return fam
    return f or "unknown"


def claims_for_ticker(notes: list, *, since: str = None) -> list:
    """Diff each filing against the previous one from the same filer AND THE SAME FORM
    FAMILY. Spec §4.4 trap, seen in the data: AMD's 10-K MD&A is ~174k chars against its
    10-Q at ~30k, so a sequential diff reports the whole annual section as "new" every
    year (and the first 10-Q after it as new again). 10-Q diffs against the prior 10-Q,
    10-K against the prior 10-K.

    `since` bounds which filings may EMIT, not which may be read: the filing immediately
    before the window is still needed as a diff baseline, or the first in-window quarter
    would diff against nothing and dump its whole MD&A.
    """
    claims = []
    families = collections.defaultdict(list)
    for n in notes:
        families[form_family(n.get("form_type"))].append(n)
    for fam in sorted(families):
        ordered = sorted(families[fam], key=lambda n: n["filed_date"])
    ...
    claims.sort(key=lambda c: (c["first_evidence_date"], c["document_id"]))
    return claims
